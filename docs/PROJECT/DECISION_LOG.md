...
| D-021 | Architecture | Proposed | Use a transactional modular control plane with relational authoritative state and an append-only event outbox written atomically with state | Model execution, external effects, projections, and delivery stay outside short state transactions; C4 boundaries await behavior review | Sequence 05 behavior analysis and `codex-design` risk consultation, 2026-08-23 |
